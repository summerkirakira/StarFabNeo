...
def main():
    ...
    theme = settings.value("theme", 'LIGHT_VS')
    try:
        if os.path.isfile(theme):
            app.setStyleSheet(qtvsc.load_stylesheet(theme))
        else:
            app.setStyleSheet(qtvsc.load_stylesheet(qtvsc.Theme[f'{theme}']))
    except Exception:
        logger.exception(f'Failed to load saved theme "{theme}", falling back to default')
        app.setStyleSheet(qtvsc.load_stylesheet(qtvsc.Theme['LIGHT_VS']))
        settings.setValue('theme', 'LIGHT_VS')
