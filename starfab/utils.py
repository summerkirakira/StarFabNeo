...
import importlib
import subprocess
from scdatatools.engine.textures.converter import convert_buffer, ConverterUtility, ConversionError
...
class ImageConverter:
    def __init__(self):
        ...
        self.converter = ConverterUtility.texconv if self.texconv else ConverterUtility.compressonator

    @property
    def converter_bin(self):
        return self.texconv if self.converter == ConverterUtility.texconv else self.compressonatorcli
    ...
    def convert_buffer(self, inbuf, in_format, out_format='tif') -> bytes:
        ...
        try:
            buf, msg = convert_buffer(inbuf, in_format=in_format, out_format=out_format,
                                          converter=self.converter, converter_bin=self.converter_bin)
