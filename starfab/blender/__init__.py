...
class CheckBlenderVersions(qtc.QRunnable):
    ...
    def run(self):
        self.manager.update_available.emit(available_blender_installations(compatible_only=True))
...
class BlenderManager(QObject):
    ...
    def __init__(self, starfab):
        ...
        self.starfab.actionInstall_Blender_Add_on.triggered.connect(self._handle_install_addon)
    ...
    def _handle_install_addon(self):
        ...
        try:
            ...
            qm.information(get_starfab(), 'StarFab Blender Addon',
            ...
            qm.warning(get_starfab(), 'StarFab Blender Addon', f'Blender addon not supported on this platform')
    ...
    def _handle_connection_approval(self, request_id):
        ...
        qm.raise_()
        ret = qm.question(get_starfab(), '', f"Accept new BlenderLink connection from process {request_id}?",
                          qm.Yes | qm.No)
        if ret == qm.Yes:
    ...
    def _handle_statusbar_label_clicked(self):
        if self._blenderlink_port > 0:
            ...
            self.starfab.statusBar.showMessage(f'Blender Link port {self._blenderlink_port} copied to clipboard')
    ...
    def start_blenderlink(self):
        if self.ensure_blenderlink_is_running():
            self.starfab.statusBar.showMessage(f'Blender Link is running on port {self._blenderlink_port}')
