...
class BlueprintExportLog(qtw.QDialog):
    def __init__(self, starfab, outdir: typing.Union[str, Path], items: typing.List[ExtractionItem],
                 create_entity_dir: bool = True, output_model_log: bool = False, export_options: dict = None):
    ...
    def _output_monitor(self, msg, entity, console, default_fmt, log_file, model_log_file, overview_console,
                        verbose=False, progress=None, total=None, level=logging.INFO, exc_info=None):
        ...
        if (model_log_file and msg.startswith('zstd |') and
                any(msg.casefold().endswith(_) for _ in CGF_CONVERTER_MODEL_EXTS)):
    ...
    def extract_entities(self) -> None:
        ...
        for i, item in enumerate(self.items):
            ...
            try:
                ...
                self.setWindowTitle(f'Extracting Entity {i + 1}/{len(self.items)}: {item.name}')
                output_dir = self.outdir / item.name if self.create_entity_dir else self.outdir
                logfile = output_dir / f'{item.name}_{datetime.now().strftime("%Y_%m_%d-%H_%M_%S")}.extraction.log'
                logfile.parent.mkdir(parents=True, exist_ok=True)

                if self.output_model_log:
                    model_log = (output_dir / f'{datetime.now().strftime("%Y_%m_%d-%H_%M_%S")}_{item.name}'
                                              f'.extracted_models.log').open('w')

                with logfile.open('w') as log:
                    monitor = partial(self._output_monitor, console=console, entity=item.name, default_fmt=default_fmt,
                                      verbose=self.export_options.get('verbose', False), log_file=log,
                                      model_log_file=model_log, overview_console=overview_console)
                    try:
                        with log_time(f'Generating Blueprint for {item.name}',
                                      partial(monitor, level=logging.CRITICAL)):
                            bp = item.bp_generator(self.starfab.sc, item.object, monitor=monitor)
                            bp_file = (output_dir / item.name).with_suffix('.scbp')
                            bp.dump(bp_file.open('w'))
                        with log_time('Extracting blueprint', partial(monitor, level=logging.CRITICAL)):
