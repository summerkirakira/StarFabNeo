...
class SOCExporterSortFilter(P4KSortFilterProxyModelArchive):
    def __init__(self, *args, **kwargs):
        ...
        self._filters = [
            (operator.and_, lambda i: i.path.suffix.lower() == '.socpak')
        ]
...
class SOCSelector(P4KContentSelector):
    ...
    def checked_items(self):
        return [
            ExtractionItem(name=_.name, object=_.info, bp_generator=blueprint_from_socpak)
            for _ in self.sc_tree_model.checked_items if _.info is not None
        ]

    @qtc.Slot()
    def _handle_p4k_loaded(self):
        self.sc_tree_model.root_item = self.sc_tree_model.itemForPath('Data/ObjectContainers')
