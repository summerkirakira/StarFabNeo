...
class P4KView(StarFabSearchableTreeWidget):
    def __init__(self, *args, **kwargs):
        ...
        convert_wem.triggered.connect(
            partial(self.ctx_manager.handle_action, "convert_wem")
        )
        ...
        if action == "extract":
            ...
            if edir:
                ...
                self.starfab.task_started.emit(
                    "convert_wem", f"Converting to {edir}", 0, total
                )
                for i, item in enumerate(selected_items):
                    ...
                    try:
                        ...
                        logger.exception(
                            f"Failed to convert wem {item.path}", exc_info=e
                        )
