...
from starfab.gui.widgets.dock_widgets.common import StarFabSearchableTreeWidget, StarFabSearchableTreeFilterWidget
...
class DCBFilterWidget(StarFabSearchableTreeFilterWidget):
    ...
    def __init__(self, parent):
        ...
        if starfab.sc is not None:
            ...
            self._type_completer = qtw.QCompleter(sorted(starfab.sc.datacore.record_types))
    ...
    def compile_filter(self) -> (typing.Callable, typing.Callable):
        ...
        if not self.tagbar.tags or starfab.sc is None:
            ...
            return op, partial(_filter_tags, method=any, tags=self.tagbar.tags, tdb=starfab.sc.tag_database)
        elif filter_type == 'has_all_tags':
            return op, partial(_filter_tags, method=all, tags=self.tagbar.tags, tdb=starfab.sc.tag_database)
...
class DCBTreeWidget(StarFabSearchableTreeWidget):
    ...
    def _handle_item_action(self, item, model, index):
        ...
        if isinstance(item, DCBItem) and item.record is not None:
            ...
            self.starfab.add_tab_widget(item.path, widget, item.name, tooltip=item.path.as_posix())
            # TODO: error dialog
    ...
    def extract_items(self, items):
        ...
        if edir:
            total = len(items)
            self.starfab.task_started.emit('extract_dcb', f'Extracting to {edir.name}', 0, total)
            t = time.time()
            for i, item in enumerate(items):
                if (time.time() - t) > 0.5:
                    self.starfab.update_status_progress.emit('extract_dcb', i, 0, total,
