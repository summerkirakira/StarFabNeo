...
class RunDialog(qtw.QDialog):
    def __init__(self, starfab):
        ...
        icon = qtg.QIcon()
        icon.addFile(str(RES_PATH / 'starfab.ico'), qtc.QSize(), qtg.QIcon.Normal, qtg.QIcon.Off)
        ...
        self.autoOpenMostRecent.setChecked(parse_bool(self.starfab.settings.value('autoOpenRecent', False)))
    ...
    def _handle_quick_load(self):
        ...
        if self._ptu_data:
            self.pushButton_quick_sc_ptu.clicked.connect(lambda: self._handle_load(self._ptu_data))
        if self._live_data:
            self.pushButton_quick_sc_live.clicked.connect(lambda: self._handle_load(self._live_data))
    ...
    def _handle_browse_file(self):
        p4k_file, _ = qtw.QFileDialog.getOpenFileName(self, "Select Star Citizen Data.p4k", f'{self._browse_path}',
                                                      "P4K (*.p4k)")
