...
class ExportRunner(qtc.QRunnable):
    def __init__(self, p4k_files: typing.List[P4KInfo], outdir: typing.Union[Path, str], save_to: bool = False,
    ...
    def run(self) -> None:
        ...
        self.starfab.task_started.emit(task_id, f'Extracting to {self.outdir}', 0, len(self.p4k_files))
        ...
        try:
            self.starfab.sc_manager.sc.p4k.extractall(
                members=self.p4k_files, path=self.outdir, monitor=_monitor, save_to=self.save_to,
                overwrite=self.export_options.get('overwrite', False),
                converters=self.export_options.get('converters', []), converter_options=self.export_options
            )
        except Exception as e:
...
class PathArchiveTreeModelLoader(qtc.QRunnable):
    def __init__(self, model, item_cls, task_name='', task_status_msg='', load_limit=-1, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.signals.cancel.connect(self._handle_cancel, qtc.Qt.BlockingQueuedConnection)
    ...
    def run(self):
        ...
        if self.task_status_message:
            self.starfab.task_started.emit(self.task_name, self.task_status_message, 0, 1)

        items = self.items_to_load()

        if self.task_status_message:
            self.starfab.update_status_progress.emit(self.task_name, 0, 0, len(items), '')
        ...
        for i, f in enumerate(items):
            ...
            if (time.time() - t) > 0.5:
                if self.task_status_message:
                    self.starfab.update_status_progress.emit(self.task_name, i, 0, 0, '')
                t = time.time()
...
class CheckableModelWrapper(PathArchiveTreeModel):
    ...
    def setData(self, index, value, role=qtc.Qt.EditRole):
        if role == qtc.Qt.CheckStateRole and index.column() == self.checkbox_column:
            item = index.internalPointer()
            self._sync_checked(item, qtc.Qt.Checked if value else qtc.Qt.Unchecked, index)
            self._update_parent(item.parent)
        return True
...
class ThreadLoadedPathArchiveTreeModel(PathArchiveTreeModel):
    ...
    def __init__(self, archive=None, columns=None, item_cls=None, parent=None, loader_cls=PathArchiveTreeModelLoader,
                 loader_task_name='', loader_task_status_msg=''):
    ...
    def load(self, archive, task_name='', task_status_msg=''):
        ...
        self.archive = archive
        self._loader = self._loader_cls(self, item_cls=self._item_cls,
                                        task_name=task_name, task_status_msg=task_status_msg)
