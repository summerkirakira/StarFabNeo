...
class _SCLoader(qtc.QRunnable):
    ...
    def run(self):
        with log_time(f'Loading {self.sc.game_folder} ({self.sc.version_label})', logger.info):
            t = time.time()

            def p4k_load_monitor(msg, progress, total):
                nonlocal t
                if (time.time() - t) > 1:
                    self.signals.update_status_progress.emit('load_p4k', progress / 1024 // 1024, None,
                                                             total / 1024 // 1024, None)
                    t = time.time()
...
class StarCitizenManager(qtc.QObject):
    ...
    def _loaded(self):
        ...
        self.datacore_model.load(self.sc)  # datacore loader expects the StarCitizen object
    ...
    def _load_sc(self, game_folder: typing.Union[str, Path], p4k_file='Data.p4k'):
        ...
        loader.signals.update_status_progress.connect(self._starfab.update_status_progress)
