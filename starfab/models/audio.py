...
from starfab.models.common import PathArchiveTreeSortFilterProxyModel, PathArchiveTreeModelLoader, \
    PathArchiveTreeItem, ContentItem, ThreadLoadedPathArchiveTreeModel
...
class AudioTreeLoader(PathArchiveTreeModelLoader):
    def items_to_load(self):
        ...
        self.starfab.task_started.emit('init_gameaudio', 'Initializing Game Audio', 0, len(ga_files))
...
class AudioTreeItem(PathArchiveTreeItem, ContentItem):
    ...
    def wems(self):
        if self.atl_name and not self._wems_loaded:
            self._wems = list(get_starfab().sc.wwise.wems_for_atl_name(self.atl_name).keys())
...
class AudioTreeModel(ThreadLoadedPathArchiveTreeModel):
    def __init__(self, sc_manager):
        ...
        super().__init__(archive=None, columns=SCAUDIOVIEWW_COLUMNS, item_cls=AudioTreeItem, parent=sc_manager,
                         loader_cls=AudioTreeLoader, loader_task_name='load_audio_model',
