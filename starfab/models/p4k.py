...
class P4KItem(PathArchiveTreeItem, ContentItem):
    ...
    def date_modified(self):
        ...
        if self.raw_time is not None:
            return qtc.QDateTime(
                *self.raw_time
            )  # .toString(qtc.Qt.DateFormat.SystemLocaleDate)
        return ""
    ...
    def save_to(self, extract_path):
        self.model.archive.save_to(
            str(self.path.as_posix()), extract_path, convert_cryxml=True
        )
...
class P4KModelLoader(PathArchiveTreeModelLoader):
    ...
    def load_item(self, item):
        self._item_cls(
            item.filename,
            model=self.model,
            parent=self.model.parentForPath(item.filename),
        )
...
class P4KModel(ThreadLoadedPathArchiveTreeModel):
    def __init__(self, sc_manager):
        ...
        super().__init__(
            None,
            columns=P4K_MODEL_COLUMNS,
            item_cls=P4KItem,
            parent=sc_manager,
            loader_cls=P4KModelLoader,
            loader_task_name="load_p4k_model",
