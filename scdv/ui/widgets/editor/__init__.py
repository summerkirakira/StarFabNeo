...
SUPPORTED_EDITOR_FORMATS = ['json', 'cfg', 'ini', 'txt', 'xml', 'log']
html = """
...
init_js = """
    ...
    new QWebChannel(qt.webChannelTransport, function(channel) {
        var scdv = channel.objects.scdv;
        
        scdv.select_all.connect(function() {
            editor.session.selection.selectAll(); 
        })
        
        scdv.set_value.connect(function(text) {
            editor.session.setValue(text)
        })
        
        scdv.save.connect(function(filename) {
            var blob = new Blob([editor.session.getValue()], {type: "text/plain;charset=utf-8"});
            saveAs(blob, filename);
        })
        
        scdv.append_text.connect(function(text) {
...
class Editor(QWebEngineView):
    ...
    def __init__(self, editor_item, theme=DEFAULT_THEME, *args, **kwargs):
        ...
        self.channel.registerObject("scdv", self.ace)
    ...
    def _on_ace_ready(self):
        try:
            self.ace.set_value.emit(self.editor_item.contents().read().decode('utf-8'))
